...
lr_decay_iters = 18000 # Giảm dần về cuối
...
def save_checkpoint(model, optimizer, scheduler, step, loss, path="/content/drive/MyDrive/my_model"):
    ...
    # Lưu file kèm số bước để không bị ghi đè
    checkpoint_name = f"checkpoint_step_{step}.pt"
    full_path = os.path.join(path, checkpoint_name)
    
    torch.save(checkpoint, full_path)
    print(f"--- Đã bảo vệ thành công thành quả tại step {step} ---")
...
# --- THIẾT LẬP ĐƯỜNG DẪN ---
save_dir = "/content/drive/MyDrive/my_model"
...
# --- BƯỚC 1 & 2: KIỂM TRA VÀ LOAD CHECKPOINT ---
if os.path.exists(ckpt_path):
    print(f"Phát hiện checkpoint tại {ckpt_path}. Đang hồi sinh...")
    ...
    start_iter = checkpoint['iter'] + 1 # Chạy tiếp từ vòng tiếp theo
    print(f"Hồi sinh thành công! Tiếp tục từ iter {start_iter}")
else:
    print("Không tìm thấy checkpoint. Bắt đầu hành trình 18.000 vòng từ số 0!")
...
for iter in range(start_iter, max_iters):
    ...
    if iter > 0 and iter % 2000 == 0:
        print(f"Đang đóng gói hành trang tại iter {iter}...")
        ckpt = {
            'iter': iter,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'scaler_state_dict': scaler.state_dict(), # Đừng quên scaler nhé!
            'loss': loss.item(),
        }
        torch.save(ckpt, ckpt_path)
        # Lưu thêm một bản riêng để kỷ niệm nếu thích
        torch.save(ckpt, f"{save_dir}/checkpoint_{iter}.pt")

    if iter % 500 == 0:
        # File nhẹ chỉ có trọng số để dùng sau này
        print(f"step {iter}: train loss {loss.item():.4f} | lr {lr:.6f}")
        torch.save(model.state_dict(), f"{save_dir}/latest_weights.pt")
...
# Sau đó lưu trực tiếp vào Drive
