...
n_routed_experts = 64    #Tổng số chuyên gia chuyên biệt.   
num_active_experts = 6  #Số chuyên gia được chọn cho mỗi token (k).
n_shared_experts = 2    #Số chuyên gia luôn luôn chạy.
moe_intermediate_size = 512 #Kích thước lớp ẩn bên trong mỗi chuyên gia
...
class SwiGLUExpert(nn.Module):
    def __init__(self, d_model, d_ff, dropout=0.1):
        super().__init__()
        self.w1 = nn.Linear(d_model, d_ff, bias=False) # Nhánh Gate
        self.w3 = nn.Linear(d_model, d_ff, bias=False) # Nhánh Up-projection
        self.w2 = nn.Linear(d_ff, d_model, bias=False) # Down-projection
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, x):
        # Công thức SwiGLU: (Swish(xW1) * xW3) * W2
...
class DeepSeekMoE(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.n_routed = config.n_routed_experts # số chuyên gia
        self.n_active = config.num_active_experts   # số chuyên gia cho mỗi tokens
        
        # 1. Gating Network (Bộ định tuyến)
        self.gate = nn.Linear(config.n_embd, self.n_routed, bias=False)

        # 2. Routed Experts (Danh sách các chuyên gia chuyên biệt)
        self.experts = nn.ModuleList([
            SwiGLUExpert(config.n_embd, config.moe_intermediate_size, config.dropout)
            for _ in range(self.n_routed)
        ])
        
        # 3. Shared Experts (Các chuyên gia dùng chung)
    ...
    def forward(self, x):
        ...
        # Tính toán Shared Experts
        shared_output = self.shared_experts(x)
        
        # Tính toán Gating scores
        logits = self.gate(x_flat) # (B*T, n_routed)
        scores = F.softmax(logits, dim=-1)
        
        # Chọn Top-K chuyên gia
        weights, selected_experts = torch.topk(scores, self.n_active, dim=-1)
        # Normalize weights để tổng bằng 1: g_i = w_i / sum(w)
        weights = weights / weights.sum(dim=-1, keepdim=True) # Normalize lại trọng số
        
        # Tổng hợp kết quả từ các Routed Experts
        # (Trong thực tế, bước này cần tối ưu bằng cách nhóm các token cùng expert)
        routed_output = torch.zeros_like(x_flat)
        for i in range(self.n_routed):
            # Tìm xem token nào chọn expert thứ i này
            mask = (selected_experts == i)
            if mask.any():
                # Lấy chỉ số hàng (token) và cột (vị trí trong Top-K)
                token_indices, topk_indices = mask.nonzero(as_tuple=True)
                
                # Trích xuất các token cần xử lý
                selected_tokens = x_flat[token_indices]
                
                # Tính toán qua Expert i và nhân với trọng số tương ứng
                expert_output = self.experts[i](selected_tokens)
                expert_weights = weights[token_indices, topk_indices].unsqueeze(-1)
                
                # Cộng dồn vào kết quả cuối
                routed_output.index_add_(0, token_indices, expert_output * expert_weights)
        
        return shared_output + routed_output.view(batch, seq_len, d_model)
