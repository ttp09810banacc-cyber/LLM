...
class MLALayer(nn.Module):
    tril: torch.Tensor
    def __init__(self, n_embd, n_head, head_size, kv_lora_rank, q_lora_rank, rope_dim, config):
        super().__init__()
        self.n_head = n_head
        ...
        self.freqs_cis = precompute_freqs_cis(
            config.rope_dim, # Thường là một phần của head_dim
            config.block_size * 2 # Nhân đôi để an toàn
        )
...
class BigramLanguageModel(nn.Module):
    # hàm khởi tạo
    def __init__(self, vocab_size, n_layer, n_head, n_embd, block_size, dropout, kv_lora_rank, q_lora_rank, rope_dim):
        super().__init__()
...
model = BigramLanguageModel(vocab_size, n_layer, n_head, n_embd, block_size, dropout, kv_lora_rank, q_lora_rank, rope_dim)
m = model.to(device)
